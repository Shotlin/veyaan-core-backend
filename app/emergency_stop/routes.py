...
async def get_status(
    ...
    if result:
        return ApiResponse(
            data=EmergencyStopStatusResponse(
...
@router.post(
    "/activate",
    response_model=ApiResponse[EmergencyStopResponse],
...
async def activate(
    ...
    if request.confirmation != "ACTIVATE_EMERGENCY_STOP":
        raise ApiError(
            ...
            "Confirmation phrase must be ACTIVATE_EMERGENCY_STOP",
            status_code=400,
        )

    result = await service.activate(
        current_user.id, request.reason or "No reason provided", current_user.id
    )
...
async def release(
    ...
    if not result:
        raise ApiError(
            ErrorCode.EMERGENCY_STOP_NOT_ACTIVE, "Emergency stop not active", status_code=404
        )
    return ApiResponse(data=EmergencyStopResponse.model_validate(result))
